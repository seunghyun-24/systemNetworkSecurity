...
/* */
std::string mac_ADR(uint8_t* macAddress) {
    char buf[23] = {};
...
std::string ip_ADR(in_addr ipAddress) {
    char buf[16] = {};
...
std::string port_num(uint16_t portNumber) {
    char buf[6] = {};
...
std::string get_data(uint8_t* data, uint8_t pre_len) {
    char buf[50] = {};
    
    uint8_t len_10 = pre_len;
...
void print_data(const u_char* packet, uint8_t ip_Len, uint8_t ip_HeaderLen, uint8_t tcpOffset) {
    ...
    printf("%s", get_data(data, pre_len).c_str());
}
/* */
