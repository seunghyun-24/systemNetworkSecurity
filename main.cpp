...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        EthHdr* ethH = (struct EthHdr*)packet;
        ArpHdr* arpH = (ArpHdr*)(packet_sizeof(EthHdr));
        ...
        for(auto& a : victimPairs) {
            for (int i = 0; pair < i; i++) {
                sendArpReply(pcap, myMac, targetIp[i], mapMac.find(a.senderIp[i])->second, a.senderIp[i]);
            }
        }
    }
